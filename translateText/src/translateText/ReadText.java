...
public class   ReadText {
	...
	public void loadfulldate(String[][] fullList, int  howMuch)//отправляем строку на отображение
	{
	int w=0;
		for(int n=0;n<howMuch;n++ )
		{
			
				nextS0=fullList[0][n].split(":")[1];//поделить строку по двоеточиям
				nextS1=fullList[1][n].split(":")[1];
				nextS2=fullList[2][n].split(":")[1];
				nextS3=fullList[3][n].split(":")[1];
				nextS4=fullList[4][n].split(":")[1];
				
				//отправить на вывод строки
				UnloadingFiles UnloadingFiles=new UnloadingFiles(nextS0,nextS1,nextS2,nextS3,nextS4);
				fullList2[n]=UnloadingFiles.getString();//формирование списка
				
			}
		MyFreme	frame=new MyFreme(fullList2);//вывод на панель данных
